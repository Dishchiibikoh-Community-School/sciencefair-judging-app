...
> Last major update: 2026-04-09 (session 2) — added Project List Share feature + two bug fixes: (1) Admin generates a shareable link from the Projects tab; teachers open it read-only, no login, no scores. Token stored in `app_settings` key `project_list_token`. URL param: `?projects=TOKEN`. New view `"public-projects"` renders projects grouped by department with color-coded badges. State: `projListToken`, `projListCopied`, `projListValid`, `projListChecked`. Functions: `generateProjListLink()`, `revokeProjListLink()`, `projListUrl()`. `loadSettings()` also reads `project_list_token`. `urlProjListToken` computed at module load (same pattern as `urlRegToken`). (2) **Bug fix — session redirect:** Added `if (urlProjListToken) return` guard to both the instant-cache-restore and session-sync `useEffect`s (same guard already existed for `urlRegToken`) — without this, any user with a saved judge session in localStorage would be silently redirected away from `public-projects`. (3) **Bug fix — token validation:** `public-projects` now uses dedicated state `projListValid`/`projListChecked` set by a direct `app_settings` DB lookup in `init()` (same pattern as `regTokenChecked`/`regTokenData` for registration links) — the previous approach relied on `loadSettings()` which could silently return nothing if the row didn't exist, causing "Link Unavailable" even with a valid link. **Important:** The "Preview Page" button uses `window.open(projListUrl(), "_blank")` — NOT `setView(...)` — because `urlProjListToken` is only set from the URL param at module load; navigating internally would always show "Link Unavailable".
>
