...
> Last major update: 2026-04-09 (session 2) — added Project List Share feature: admin generates a shareable link from the Projects tab that lets teachers review the project list (no login, no scores shown). Token stored in `app_settings` key `project_list_token`. URL param: `?projects=TOKEN`. New view `"public-projects"` validates the URL token against the stored token and renders projects grouped by department with color-coded badges. State: `projListToken`, `projListCopied`. Functions: `generateProjListLink()`, `revokeProjListLink()`, `projListUrl()`. `loadSettings()` now also reads `project_list_token`. `urlProjListToken` computed at module load (same pattern as `urlRegToken`). **Important:** The "Preview Page" button uses `window.open(projListUrl(), "_blank")` — NOT `setView("public-projects")` — because the `public-projects` view validates `urlProjListToken === projListToken` (URL param must be present); navigating internally without the param would always show "Link Unavailable".
>
