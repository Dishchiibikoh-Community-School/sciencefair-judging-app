...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .glow{position:fixed;top:-200px;left:50%;transform:translateX(-50%);width:700px;height:500px;
    background:radial-gradient(ellipse at 50% 0%,#f0a50015 0%,transparent 70%);pointer-events:none;}
  .glow.purple{background:radial-gradient(ellipse at 50% 0%,#7c3aed15 0%,transparent 70%);}
  .land-badge{font-family:var(--ff-m);font-size:.7rem;letter-spacing:.18em;color:var(--gold);
    border:1px solid var(--gold2);border-radius:100px;padding:.3rem 1rem;margin-bottom:2rem;display:inline-block;}
...
export default function App() {
  ...
  const [judges,     setJudges]  = useState(SEED_JUDGES);
  ...
  // IT logs state
  const [itLogs,       setItLogs]       = useState(SEED_IT);
  ...
  function shareUrl() { return `https://sciencefair.school.edu/results?token=${shareToken}`; }
  ...
  // Helpers
  function addLog(msg) { setLog(p => [{ time: Date.now(), msg }, ...p]); }

  function addItLog(level, module, event, detail, payload = {}) {
    setItLogs(p => [{ id: itId(), ts: Date.now(), level, module, event, detail, payload }, ...p]);
  }
    ...
    return (
      <div className="app"><style>{CSS}</style>
        <div className="center"><div className="inner">
          ...
          <button className="btn sec" style={{ marginTop:".85rem" }} onClick={() => { setJudge(null); setView("landing"); }}>Sign Out</button>
        </div></div>
      </div>
    );
  }
          ...
          <div className="adm-side">
            ...
            <div className="nav-it" style={{ color:locked?"var(--red)":"var(--green)" }}
              onClick={() => {
                ...
                addItLog(next?"WARN":"INFO","ADMIN", next?"JUDGING_LOCKED":"JUDGING_UNLOCKED", next?"Admin locked judging — no more score submissions allowed":"Admin unlocked judging — submissions re-enabled",{ lockedBy:"admin", timestamp:fmtISO(Date.now()) });
              }}>
